...
COMPUTE_CAP=60
NVCC=nvcc
NVCCFLAGS=-std=c++11 -arch=sm_60 -gencode=arch=compute_30,code=sm_30 -gencode=arch=compute_50,code=sm_50 -gencode=arch=compute_52,code=sm_52 -gencode=arch=compute_60,code=sm_60 -gencode=arch=compute_61,code=sm_61 -gencode=arch=compute_61,code=compute_61 -Xptxas="-v" -Xcompiler "${CXXFLAGS}" -Wno-deprecated-gpu-targets
